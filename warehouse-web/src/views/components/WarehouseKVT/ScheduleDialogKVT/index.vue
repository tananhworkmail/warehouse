<template>
  <el-dialog v-model="dialogVisible" :title="title" :fullscreen="true">
    ...
    <el-tabs v-model="tabsActiveName">
      <el-tab-pane label="KVT" name="KVT">
        <SpreadsheetViewer
          v-if="tabsActiveName === 'KVT' && fileBlobUrl"
          :fileURL="fileBlobUrl"
        />
      </el-tab-pane>
...
/* ===== FIX CỨNG ===== */
const buildingNo = "KVT";
const FILE_NAME = "NCU_KVT.xlsx";

const tabsActiveName = ref("KVT");
...
const loadScheduleFile = async () => {
  try {
    ...
    ElMessage.error("Không tải được file Schedule");
  } finally {
    loading.value = false;
  }
};
